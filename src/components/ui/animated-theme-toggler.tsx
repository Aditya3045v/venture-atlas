...
interface AnimatedThemeTogglerProps extends React.ComponentPropsWithoutRef<'button'> {
   ...
   * (e.g. `next-themes` or custom ThemeProvider) and this component will not write to localStorage.
   */
  theme?: 'light' | 'dark';
...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      ...
      // Toggle class synchronously for View Transitions API snapshot
      if (newTheme) {
      ...
      if (isControlled) {
        ...
        localStorage.setItem('va_dark_theme', String(newTheme));
      }
    };
  ...
  return (
    <button
      ...
      onClick={toggleTheme}
      className={cn(
        'p-2.5 rounded-xl text-text-secondary hover:text-text-primary hover:bg-surface-muted transition-all active:scale-90 flex items-center justify-center',
        className
      )}
